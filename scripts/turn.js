...
function run(){ /*将定时器里的函数提取到外部*/
    ...
    var timer = setInterval(run,5000); /*定义定时器，使图片每隔5s更换一次*/
...
function jump(){
    var div_jump=document.getElementById("jump");
    var div_middle=document.getElementById("middle");
    var item=div_jump.getElementsByClassName("toQABar");
    var label=div_middle.getElementsByTagName("h2");
    var top=item[item.length-1];
    for(i=0;i<label.length;i++){
        attach(item[i],label[i]);
    }
    top.onclick=function(){window.scrollTo({"behavior": "smooth", "top": 0,"left":0});};
   
}

function attach(item,label){
    item.onclick=function(){
        window.scrollTo({"behavior": "smooth", "top": label.offsetTop,"left":label.offsetLeft});
    }
}
...
function search(){
    ...
    sure.onclick=function(){
        ...
        for( i=0;i<p.length;i++){
            var temp=match(input,p[i]);
            if(temp!=-1){
                count++;
            }
            }
        alert("找到"+count+"条符合要求的项目");
        setTimeout("recover()",5000);
        }
    }

function match(input,text){
    if(text.lastChild.nodeValue.indexOf(input)!=-1){
        window.scrollTo({"behavior": "smooth", "top": text.offsetTop,"left":text.offsetLeft});
